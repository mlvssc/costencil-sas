...
int base(long t0, long t1, vec3 v0, vec3 v1, vec3 rv0, vec3 rv1)
{
  ...
  REAL *darrays[2];
  {
    size_t fetchsize = (v1z.z-v0z.z)*(n3d.x+2)*(n3d.y+2);
    
    // fetch regions
    int i;
    for (i = 0; i < 2; i++) {
      darrays[i] = (REAL *)mm_fetch((void*)&arrays[i][IDX(v0z.x, v0z.y, v0z.z, n3d.x, n3d.y)],
				    sizeof(REAL)*fetchsize);
    }
    
    for (i = 0; i < 2; i++) {
      assert(darrays[i] != NULL);
    }
  }
...
int recTB(long t0, long t1, vec3 v0, vec3 v1, vec3 rv0, vec3 rv1)
{
  ...
  REAL *darrays[2];
  {
    ...
    for (i = 0; i < 2; i++) {
      if (darrays[i] == NULL) dataflag = 0; // not available yet
    }
  }
  // now [z0z, z1z) are accessible by GPU on darrays, if (dataflag == 1) 
